...
var device_id=ALICOOKIE.match(/device_id=(.*?)[\s;]/)[1];
    ...
    JSON.stringify([{name:"原始文件",url:playurl,head:{"Referer":"https://www.aliyundrive.com/"}}]);
        ...
        var drive_id=getVar("地址").split("?wd=")[1].split("###")[1];
        var u=getVar("地址").split("?wd=")[1].split("###")[0];
        var rsign=getHttp(JSON.stringify({url:"http://113.107.160.110:3000/sign/"+access_token+'/'+mydrive_id+'/'+user_id+'/'+device_id+"/sign.json"}));
        var code=getHttp(JSON.stringify({url:"https://api.aliyundrive.com/v2/file/get_download_url",postJson:JSON.stringify({drive_id:drive_id,file_id:file_id,expires_sec:0}),head:JSON.parse(rsign)}));
