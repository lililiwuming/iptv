...
if(getVar("地址").indexOf("$$")!=-1){
    ...
    if(JSON.parse(code).audio_template_list){
        ...
        var 转码HQ='http://113.107.160.110:3000/apis/yun-audio/'+share_id+'/'+file_id+'/'+access_token+'/'+share_token+'/HQ/master.mp3';
        JSON.stringify([{name:"不限ip转码",url:转码HQ},{name:"原始文件",url:JSON.parse(code).audio_template_list[JSON.parse(code).audio_template_list.length-1].url,head:{"User-Agent":"okhttp/4.2.2","Connection":"Keep-Alive","Referer":"https://aliyundrive.com/"}}]);
    }else{
