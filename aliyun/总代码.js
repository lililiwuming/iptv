...
if(getVar("地址").indexOf("$$")!=-1){
    ...
    //var resp=JZ(JSON.stringify({url:JSON.parse(code).download_url,redirect:false,head:{"Referer":"https://www.aliyundrive.com/"}}));
            ...
            var 备用='http://38.55.98.61:3000/apis/yun-play/'+share_id+'/'+file_id+'/'+access_token+'/'+share_token+'/FHD/index.m3u8';
        JSON.stringify([{name:"原始文件播放",url:JSON.parse(code).download_url,head:{"Referer":"https://www.aliyundrive.com/"}},{name:"不限ip转码",url:备用},{name:"转码1080P可投屏",url:转码1080},{name:"转码720P可投屏",url:转码720}]);
    }
    }
}else{
var cm=android.webkit.CookieManager.getInstance();
...
alert("请重新登陆阿里云盘网页");
    ...
    var 备用='http://38.55.98.61:3000/apis/my-yun-play/'+file_id+'/'+access_token+'/'+share_token+'/FHD/index.m3u8';
