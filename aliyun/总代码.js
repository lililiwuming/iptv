...
if(getVar("地址").indexOf("$$")!=-1){
    ...
    if(JSON.parse(code).audio_template_list){
        ...
        JSON.stringify([{name:"原始文件播放",url:JSON.parse(code).download_url,head:{"Referer":"https://www.aliyundrive.com/","User-Agent":"Lavf/57.83.100"}},{name:"本地高画质转码(会员2K)",url:本地转码2K会员专享},{name:"720P转码",url:转码720},{name:"高画质转码(会员2K)",url:转码2K会员专享}]);
...
var refresh_token=ALICOOKIE.match(/refresh_token=(.*?)[\s;]/)[1];
var code=getHttp(JSON.stringify({url:"https://open.aliyundrive.com/oauth/access_token",postJson:JSON.stringify({client_secret:"dbw2OtmVEeuUvIptb1Coyg",client_id:"YNxT9w7GMdWvEOKa",refresh_token:refresh_token,grant_type:"refresh_token"})}));
alert(code)
if(JSON.parse(code).access_token){
var access_token=JSON.parse(code).access_token;
var mydrive_id=ALICOOKIE.match(/drive_id=(.*?)[\s;]/)[1];
...
var HEAD=JSON.stringify({"Authorization":'Bearer '+access_token});
    ...
    JSON.stringify([{name:"原始文件",url:playurl,head:{"Referer":"https://www.aliyundrive.com/"}}]);
        ...
        JSON.stringify([{name:"原画",url:playurl,head:{"Referer":"https://www.aliyundrive.com/","User-Agent":"Aliapp(AYSD/4.1.2)"}},{name:"本地1080P转码",url:本地高画质转码},{name:"720P转码",url:转码720},{name:"高画质转码",url:高画质转码}]);
        //JSON.stringify([{name:"原始文件",url:playurl,head:{"Referer":"https://www.aliyundrive.com/"}}]);
    }
}else{
    alert("登陆已过期，请重新在m浏览器登陆");
