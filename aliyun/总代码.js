...
if(getVar("地址").indexOf("$$")!=-1){
    ...
    if(JSON.parse(code).audio_template_list){
        ...
        JSON.stringify({url:JSON.parse(code).audio_template_list[JSON.parse(code).audio_template_list.length-1].url,head:{"User-Agent":"okhttp/4.2.2","Connection":"Keep-Alive","Referer":"https://aliyundrive.com/"}});
    }else{
