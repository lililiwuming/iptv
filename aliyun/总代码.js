...
if(getVar("地址").indexOf("$$")!=-1){
    ...
    if(类型=="audio"){
    var code=getHttp(JSON.stringify({url:"https://api.aliyundrive.com/v2/file/get_share_link_download_url",head:{"Authorization":access_token,"X-Share-Token":share_token},postJson:JSON.stringify({share_id:share_id,get_audio_play_info:true,file_id:file_id,expires_sec:0})}));
    }else{
    ...
    if(JSON.parse(code).audio_template_list){
        ...
        JSON.stringify([{name:"原始文件",url:JSON.parse(code).audio_template_list[JSON.parse(code).audio_template_list.length-1].url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/","User-Agent":"Aliapp(AYSD/4.1.2)","User-Agent":"Dalvik/2.1.0"}},{name:"高音质转码",url:转码HQ}]);
    }else{
        ...
        JSON.stringify([{name:"原始文件播放",url:JSON.parse(code).download_url+'&xFormat=unknown',head:{"User-Agent":"Lavf/57.83.100"}},{name:"本地高画质转码(会员2K)",url:本地转码2K会员专享},{name:"720P转码",url:转码720},{name:"高画质转码(会员2K)",url:转码2K会员专享}]);
...
var device_id=ALICOOKIE.match(/device_id=(.*?)[\s;]/)[1];
    ...
    JSON.stringify([{name:"原始文件",url:playurl,head:{"Referer":"https://www.aliyundrive.com/"}}]);
        ...
        var drive_id=getVar("地址").split("?wd=")[1].split("###")[1];
        var u=getVar("地址").split("?wd=")[1].split("###")[0];
        var rsign=getHttp(JSON.stringify({url:"http://113.107.160.110:3000/apis/sign/"+access_token+'/'+mydrive_id+'/'+user_id+'/'+device_id+"/sign.json"}));
        var HEAD=JSON.stringify({"Authorization":JSON.parse(rsign).authorization,"x-device-id":JSON.parse(rsign)['x-device-id'],"x-signature":JSON.parse(rsign)['x-signature']});
        var code=getHttp(JSON.stringify({url:"https://api.aliyundrive.com/v2/file/get_download_url",postJson:JSON.stringify({drive_id:drive_id,file_id:file_id,expires_sec:0}),head:JSON.parse(HEAD)}));
        ...
        JSON.stringify([{name:"原始文件播放",url:JSON.parse(code).url,head:{"Referer":"https://www.aliyundrive.com/","User-Agent":"Aliapp(AYSD/4.1.2)"}},{name:"本地1080P转码",url:本地转码1080},{name:"720P转码",url:转码720},{name:"1080P转码",url:转码1080}]);
        //JSON.stringify([{name:"原始文件",url:playurl,head:{"Referer":"https://www.aliyundrive.com/"}}]);
    }
}else{
    alert("登陆已过期，请重新在m浏览器登陆");
