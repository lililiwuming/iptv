...
/*{
    "title":"阿里小站",
    "rule":{
        "分类":'var a="影视$Movies$$图片$picture$$软件$software$$游戏$games$$音乐$music$$动漫$comic$$书籍$book$$学习$learn$$互帮互助$help$$其他$other";var a=a.split("$$");var 前="https://lxlb.ml/api/discussions?include=user,lastPostedUser,tags,tags.parent,firstPost&filter[tag]=";var 后="&sort&page[offset]=#PN#";var items=[];for(var i in a){var title=a[i].split("$")[0];var url=前+a[i].split("$")[1]+后;items.push({title:title,url:url});}JSON.stringify(items);',
        "列表规则":'var 列表=e2Arr(getCode(),".json(data)");var 地址规则=".c(https://lxlb.ml/api/discussions/).json(attributes).json(slug)";var 标题规则=".json(attributes).json(title)";var 图片规则=".json(tok)";var 简介规则=".c(发布时间：).json(attributes).json(createdAt).c(最近回复时间：).json(attributes).json(lastPostedAt)";var NEXTPAGE=e2Rex(getCode(),".json(links).json(next)");var PREPAGE=e2Rex(getCode(),".json(links).json(prev)");',
...
},*/
{
    "title":"霸王龙字幕组",
    "rule":{
        "分类":'var a="最新资源$/$$美剧$/meiju/$$韩剧$/hanju/$$电影$/电影/$$日剧$/riju/$$泰剧$/taiju/$$英剧$/yingju/$$动画$/动画/$$真人秀综艺$/真人秀综艺/$$纪录片$/纪录片/$$其他剧$/qitaju/";var a=a.split("$$");var items=[];for(var i in a){var title=a[i].split("$")[0];var url="http://t-rex.tzfile.com"+a[i].split("$")[1]+"page/#PN#";items.push({title:title,url:url});}JSON.stringify(items);',
        "列表规则":'var 列表=e2Arr(getCode(),".get(div.row div.col-lg-1-5)");var 地址规则=".get(h2 a).a(href)";var 标题规则=".c([).get(span.meta-category).t().c(]).get(h2 a).a(title)";var 图片规则=".get(img).a(data-src)";var 简介规则=".c(更新时间:).get(time).t()";var NEXTPAGE="";var PREPAGE="";',
        "详情规则":'var cm=android.webkit.CookieManager.getInstance();var COOKIE=cm.getCookie(getVar("地址").match(/http.+?\\/\\/.+?\\//)[0]);var 源码=e3("读源码({地址:"+getVar("地址")+",COOKIE:"+COOKIE+"})");alert(源码);var 正文=e2Rex(源码,".get(div.entry-content).t()");'
    }
},
{
    "title":"阿里小纸条",
