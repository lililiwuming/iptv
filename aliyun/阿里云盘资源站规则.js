...
},*/
{
    "title":"霸王龙字幕组",
    "rule":{
        "分类":'var a="最新资源$/$$美剧$/meiju/$$韩剧$/hanju/$$电影$/电影/$$日剧$/riju/$$泰剧$/taiju/$$英剧$/yingju/$$动画$/动画/$$真人秀综艺$/真人秀综艺/$$纪录片$/纪录片/$$其他剧$/qitaju/";var a=a.split("$$");var items=[];for(var i in a){var title=a[i].split("$")[0];var url="https://t-rex.tzfile.com"+a[i].split("$")[1]+"page/#PN#";items.push({title:title,url:url});}JSON.stringify(items);',
        ...
        "详情规则":'var cm=android.webkit.CookieManager.getInstance();var COOKIE=cm.getCookie(getVar("地址").match(/http.+?\\/\\/.+?\\//)[0]);var 源码=e3("读源码({地址:"+getVar("地址")+",COOKIE:"+COOKIE+"})");var 正文=e2Rex(源码,".get(div.entry-content).t()");'
    }
},
{
    "title":"阿里小纸条",
